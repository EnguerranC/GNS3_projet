...
"""
def Adressage_AS(Num_AS , Matrice_adjacence, Nombre_routeur, M_adresse) :
        ...
        for i in range(Nombre_routeur) :
            for j in range(Nombre_routeur) :
               if Matrice_adjacence[i][j] :
                  nb_connexions[i] += 1
                  nb_connexions[j] += 1
                  adresse_unique1 = config[Num_AS]["Masque_reseau"][:3]+":0:0:"+str(j+1)+"::"+"1/64"
                  adresse_unique2 = config[Num_AS]["Masque_reseau"][:3]+":0:0:"+str(j+1)+"::"+"2/64"
                  interface1 = "GigabitEthernet"+str(nb_connexions[i])+"/0"
                  interface2 = "GigabitEthernet"+str(nb_connexions[i])+"/0"
